layui.define('form', function (exports) {
  ...
  Class.prototype.tree = function (elem, children) {
    ...
    layui.each(data, function (index, item) {
      var isLazy = item.lazy ? '" data-lazy="true"' : '" data-lazy="false"';
      var hasChild = item.children && item.children.length > 0
          , packDiv = $('<div class="layui-tree-pack" ' + (item.spread ? 'style="display: block;"' : '') + '"></div>')
          , entryDiv = $(['<div data-id="' + item.id + isLazy + ' class="layui-tree-set' + (item.spread ? " layui-tree-spread" : "") + (item.checked ? " layui-tree-checkedFirst" : "") + '">'
        // ,entryDiv = $(['<div data-id="'+ item.id +'" data-lazy="' + item.lazy +'" class="layui-tree-set'+ (item.spread ? " layui-tree-spread" : "") + (item.checked ? " layui-tree-checkedFirst" : "") +'">'
