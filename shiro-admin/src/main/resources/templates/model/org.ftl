<div class="iframe_right_top">
    ...
    <div class="right_top_right">
        <@shiro.hasPermission name="org:batchDelete">
...
<div class="iframe_right_list">
    ...
    <form  class="bootstrap-frm" style="display: none;">
        ...
        <label>
            <span>名称:</span>
            <input  type="text" name="name" placeholder="请输入机构的名称" />
        </label>
        <label>
            <span>机构号 :</span>
            <input  type="text" name="code" placeholder="请输入机构号" />
        </label>
        <label>
            <span>&nbsp;</span>
...
<script>
    ...
    const list = function (pageNum,param) {
        ...
        $.ajax({
            ...
            success: function (json) {
                ...
                let ser = 1;
                for(let row of rows){
                    let object = row.sysOrg;
                    content+=`<tr id="`+object.id+`">
                            <td><input name="org" type="checkbox" value="`+object.id+`"></td>
                            <td>`+ser+++`</td>
                            <td>`+object.name+`</td>
                            <td>`+object.code+`</td>
    ...
    const out = function () {
        ...
        $('input[name="org"]:checked').each(function(){//遍历每一个名字为interest的复选框，其中选中的执行函数
