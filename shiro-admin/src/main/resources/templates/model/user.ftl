<div class="iframe_right_top">
    ...
    <div class="right_top_right">
        ...
        <@shiro.hasPermission name="user:allotRole">
            <div class="right_btn" onclick="out()">分配角色</div>
        </@shiro.hasPermission>

        <@shiro.hasPermission name="user:setpwd">
            <div class="right_btn" onclick="out()">设置密码</div>
        </@shiro.hasPermission>
    ...
    const list = function (pageNum,param) {
        ...
        $.ajax({
            ...
            success: function (json) {
                ...
                let ser = 1;
                for(let row of rows){
                    let object = row.sysUser;
                    content+=`<tr id="`+object.id+`">
                            <td><input name="user" type="checkbox" value="`+object.id+`"></td>
                            <td>`+ser+++`</td>
                            <td>`+object.username+`</td>
                            <td>`+object.nickname+`</td>
                             <td>`+object.mobile+`</td>
                             <td>`+object.email+`</td>
                             <td>`+userStatus(object.status)+`</td>
                            <td>`+renderTime(object.lastLoginTime)+`</td>
                            </tr>`;
                }
                tbody.html(content);
            }
