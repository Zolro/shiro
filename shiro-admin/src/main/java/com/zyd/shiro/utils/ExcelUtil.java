...
public class ExcelUtil<T>{
 ...
 /*   public List<T> excelToList(String sheetName, File f, Class<T> item,Map<String,String> fieldMatch,long direId) {
        List<T> list = null;
        try {
            ...
            //titleService.saveOrUpd(headers,direId);

            // 获取数据
            list = dispatch(sheet, item,fieldMatch);
    ...
    public String[] getHeaderValue(Row rowHeader, Class<T> cls,Map<String,String> fieldMacth) {
        int colNum = rowHeader.getPhysicalNumberOfCells();
        String[] headValue = new String[rowHeader.getPhysicalNumberOfCells()];
        Map<String,String> map = getFieldAnnotation(cls);
        for(String key:fieldMacth.keySet()){
            if(map.containsKey(key)){
                String value = map.get(key);
                map.remove(key);
                map.put(fieldMacth.get(key),value);
            }
        }
        for (int i = 0; i < colNum; i++) {
            String title = rowHeader.getCell(i).getStringCellValue();
    ...
    public  Map<String,String> getFieldAnnotation(Class<T> cls,Map<String,String> fieldMatch){
        Map<String,String> map = new HashMap<>();
        //当父类为null的时候说明到达了最上层的父类(Object类).
        Field[] fields = cls.getDeclaredFields();
        for(Field field:fields){
